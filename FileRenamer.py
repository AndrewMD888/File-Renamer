# First created by Andrew Dulichan on February 19, 2018, last updated by Andrew Dulichan on May 23, 2019
# SEE THE README IN MY REPOSITORY FOR DOCUMENTATION!

import os, re, filecmp, itertools
...
#--Start of filename collision checking function--
#-------------------------------------------------

# This function's purpose is to handle cases where files that get renamed end up with the same filename
# This can happen when certain characters get stripped from the original filename(s) and multiple files then end up with the same name
...
def check_filename_collision(new_filename_list, new_filename, dir, num_files_in_dir):

    if num_files_in_dir > 1:
        for current_file in new_filename_list[1:]:
            if new_filename == current_file:
                old_name = new_filename
                new_filename = "copy_of_" + new_filename
                os.rename(os.path.join(dir, old_name), os.path.join(dir, new_filename))
                check_filename_collision(new_filename_list, new_filename, dir, num_files_in_dir)

#-------------------------------------------------
#---End of filename collision checking function---
...
if not directory_files:
    ...
    for filename in directory_files:
        ...
        # Save original filenames for later comparison/usage
        ...
        # Save new filenames for later comparison/usage
        new_filename_list.append(new_filename)

    for original_filename, new_filename in zip(original_filename_list, new_filename_list):
        ...
        check_filename_collision(new_filename_list, new_filename, directory, num_files_in_directory)
